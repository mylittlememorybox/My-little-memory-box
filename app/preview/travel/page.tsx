  ...
  <div style={{ transform: `rotate(${rotation}deg)`, opacity, display: "inline-block" }}>
    <svg width="110" height="110" viewBox="0 0 160 160">
      ...
      <text fontSize="11" fill={color} fontFamily="Georgia, serif" letterSpacing="3">
        <textPath href={`#top-${city}-p`} startOffset="50%" textAnchor="middle">
          {country.toUpperCase()}
        </textPath>
      </text>
      ...
      <text fontSize="9" fill={color} fontFamily="Georgia, serif" letterSpacing="2">
        <textPath href={`#bot-${city}-p`} startOffset="50%" textAnchor="middle">
          {entryType}
        </textPath>
      </text>
  ...
  const renderPage = () => {
    ...
    if (page.key === "cover") return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", background: "linear-gradient(160deg, #0D2B4A 0%, #1A4A7A 50%, #0D3D5C 100%)", margin: "-16px", borderRadius: "12px", padding: "30px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginTop: "24px", opacity: 0.6, flexWrap: "wrap", justifyContent: "center" }}>
          {[
            { city: "ROMA", color: "#A8C4E0", rot: -8 },
            { city: "TOKYO", color: "#C4D4E8", rot: 5 },
            { city: "SANTORINI", color: "#A8C4E0", rot: -4 },
          ].map((s, i) => (
            <div key={i} style={{ transform: `rotate(${s.rot}deg)` }}>
              <svg width="55" height="55" viewBox="0 0 160 160">
                ...
                <text x="80" y="75" textAnchor="middle" fontSize="18" fill={s.color}>✈</text>
                <text x="80" y="94" textAnchor="middle" fontSize="13" fill={s.color} fontFamily="Georgia">{s.city}</text>
              </svg>
            </div>
          ))}
        </div>
    ...
    if (page.key.startsWith("trip_")) {
      ...
      return (
        <div style={{ overflowY: "auto", height: "100%", padding: "0 4px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "10px" }}>
            {[1, 2].map(i => (
              <div key={i} style={{ height: "100px", background: "rgba(26,74,122,0.08)", borderRadius: "10px", border: "2px dashed rgba(26,74,122,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #0D2B4A 0%, #1A4A7A 60%, #0D3D5C 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      ...
      <div style={{
        ...
        boxShadow: "12px 12px 40px rgba(0,0,0,0.5), -2px 0 8px rgba(0,0,0,0.2), inset -4px 0 8px rgba(0,0,0,0.08)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "32px", marginTop: "20px" }}>
        ...
        >←</button>
        <span style={{ color: "#A8C4E0", fontSize: "12px", fontFamily: "Georgia, serif", letterSpacing: "1px", maxWidth: "150px", textAlign: "center" }}>

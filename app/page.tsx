...
description:
“Κράτησε κάθε «πρώτη φορά» — το πρώτο χαμόγελο, τα πρώτα βήματα, τις πρώτες λέξεις. Ένα λεύκωμα που θα αγαπήσει να ανακαλύπτει όταν μεγαλώσει.”,
features: [
...
description:
“Μια συλλογή από τις πιο ιδιαίτερες στιγμές της σχέσης μας. Φωτογραφίες, αναμνήσεις και τα λόγια που δεν θέλουμε να ξεχάσουμε ποτέ.”,
features: [
...
description:
“Η πιο σημαντική μέρα της ζωής μας, σε κάθε λεπτομέρεια. Φωτογραφίες, συναισθήματα, ευχές και τα όσα κάναμε αυτή τη μέρα αξέχαστη.”,
features: [
...
return (
<nav
className={`fixed top-0 left-0 right-0 z-100 px-8 py-4 flex justify-center gap-40 transition-all ${ scrolled ? "bg-[rgba(249,242,236,0.96)] backdrop-blur-md shadow-sm" : "" }`}
>
...
```
  <div className="mb-8 mt-4">
    <img
      ...
      onError={(e) => {
        e.currentTarget.src =
          "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22210%22 height=%22190%22%3E%3Crect fill=%22%23F5EDE0%22 width=%22210%22 height=%22190%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 font-size=%2224%22 fill=%22%23C49090%22%3E🎀%3C/text%3E%3C/svg%3E";
      }}
    />
  </div>
  ...
  <p className="text-center text-[#B09880] max-w-sm leading-relaxed font-light text-base">
    Δημιούργησε το δικό σου Memory Box γεμάτο φωτογραφίες και λόγια αγάπης
    και χάρισε στο παιδί σου ένα προσωποποιημένο ebook παραμύθι.
  </p>
    ...
    {templates.map((box) => (
      ...
      >
        ...
        <div className="p-8 text-center">
          ...
          <h3 className="text-2xl font-serif font-normal text-[#5C3820] mb-1">
            {box.name}
          </h3>
          <p className="text-xs tracking-widest uppercase text-[#B09880] mb-4">
            {box.tagline}
          </p>
          <p className="text-sm font-light text-[#7A6055] leading-relaxed text-left mb-4">
            {box.description}
          </p>
        </div>
        ...
        <ul className="px-7 space-y-1.5 text-sm font-light text-[#7A6055]">
          {box.features.map((feat, i) => (
            <li
              key={i}
              className="flex gap-2.5 border-b border-[rgba(196,168,130,0.12)] pb-1.5 last:border-b-0"
            >
              <div
                className="w-1 h-1 rounded-full flex-shrink-0 mt-1.5"
                style={{ background: box.accentColor }}
              />
              {feat}
            </li>
          ))}
...
<h2 className="text-[#8B5E3C] font-serif text-4xl font-normal mb-2 reveal">
Πώς λειτουργεί
</h2>
...
```
  <div className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-2xl mx-auto">
    {[
      {
        num: "01",
        title: "Επίλεξε Memory Box",
        desc: "Διάλεξε αυτό που ταιριάζει στην περίσταση",
      },
      {
        num: "02",
        title: "Συμπλήρωσε το",
        desc: "Πρόσθεσε φωτογραφίες και λόγια αγάπης",
      },
      {
        num: "03",
        title: "Μοιράσου το",
        desc: "Λήψη PDF ή αποστολή ως ψηφιακό δώρο με QR code",
      },
    ].map((step) => (
      <div key={step.num} className="reveal">
        <div className="text-4xl font-serif font-light text-[rgba(196,168,130,0.35)] mb-2">
          {step.num}
        </div>
        <h3 className="text-[#8B5E3C] font-serif text-lg font-normal mb-2">
          {step.title}
        </h3>
        <p className="text-sm font-light text-[#B09880] leading-relaxed">
          {step.desc}
        </p>
      </div>
    ))}
